...
/* From http://php.net/manual/en/function.str-getcsv.php#113220 */
...
function wp_hgrant_parse_csv($str)
{
...
function wp_hgrant_parse_csv_quotes($matches)
{
    ...
    $str = str_replace("\r", "\rR", (isset($matches[3]) ? $matches[3] : ''));
    $str = str_replace("\n", "\rN", $str);
...
function wp_hgrant_parse_csv_line($line)
{
    return array_map('wp_hgrant_parse_csv_field', explode(',', $line));
}

//restore any csv-special characters that are part of the data
function wp_hgrant_parse_csv_field($field) {
